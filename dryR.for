...
C      Read input, set up grid, initial conditions.
      ...
      if (isweed_theta .eq. 1) then       ! solver output
        open(107,file= 'output/ptsTheta.out')   
  			open(112,file= 'output/Htop.out')  ! Richards surface H tracking              				
C       open(108,file= 'output/dTheta.out')
      endif
...
C 	  file 100 is 'output/h.out' 
     ...
     &                  xflux1(j,k), yflux1(j,k)
      ...
      if  (isweed_theta .eq. 1) then
C 	  file 100 is 'output/h.out' 
	      do j=1,ncol
	        do k=kbeg(j),kend(j)
	          write(112,*) j, k, r8H(j,k,nz),r8H(j,k,nz-1),
     &           isetflux          
	        enddo
	      enddo
		    write(112, 202) itp, t        
...
************************************************************************
      ...
      read(4,*) jbare,  kbare
      read(4,'(a72)') dum      
      read(4,*) isweed_theta, isweed_flux
      read(4,'(a72)') dum 
      read(4,*) iBC, tsat_min
