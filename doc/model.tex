...
Columns are \code{j,k,h,u,v,zinflmap2, xflux0,yflux0,xflux1,yflux1}. 

These variables are written to \code{h.out} for every grid cell (every j,k) when \code{myoutput} is called, every nprt timesteps (every \code{dt\_p}. 
...
To simulate runoff/runon environments on patchily-vegetated hillslopes, several aspects of the BK model have been modified (in addition to coupling to a 1D Richards solver).  This section describes the simulated domain, as illustrated in Figure XXXX.  
...
\subsection{To compile and execute \code{dry.f}: \code{read\_sim})}
\subsection{To read the Fortran output files: \code{read\_sim})}
